...
extension DataManager{
    ...
    public func addCityList(withCityList cityList: [City], closure: @escaping (_ codeResult: CodeResult, _ message: String)->()){
        ...
        for city in cityList{
            let dic: [String: Any] = [
                ...
                "name": city.name,
                "sort": city.sort,
                "localFlag": city.localFlag ? 1 : 0
                ]
            dataDic.append(dic)
        }
        
        let dic = ["data": dataDic]
    ...
    ///排序城市
    public func sortCity(withCityList cityList: [City], closure: @escaping (_ codeResult: CodeResult, _ message: String)->()){
        ...
        let jsonDic = ["data": dataDic]
        
        Session.session(withAction: Actions.sortCity, withRequestMethod: .post, withParam: jsonDic) { (codeResult, message, data) in
            DispatchQueue.main.async {
                closure(codeResult, message)
            }
        }
    ...
    ///收藏单个城市
    public func addCity(withCity city: City, closure: @escaping (_ codeResult: CodeResult, _ message: String)->()){
        let dic: [String: Any] = [
            "cityId": "\(city.id)",
            "name": city.name,
            "localFlag": city.localFlag ? 1 : 0
        ]
        Session.session(withAction: Actions.markSingleCity, withRequestMethod: .post, withParam: dic) { (codeResult, message, data) in
            DispatchQueue.main.async {
                closure(codeResult, message)
            }
        }
    ...
    //MARK:- 取消收藏城市
    public func deleteCity(withCityId cityId: Int, withIsLocal localFlag: Bool = false, closure: @escaping (_ codeResult: CodeResult, _ message: String)->()){
        let dic = [
            "id": "\(cityId)",
            "localFlag": "\(localFlag ? 1 : 0)"
        ]
        ...
        Session.session(withAction: Actions.getWeatherSetting, withRequestMethod: .post, withParam: [:]) { (codeResult, message, data) in
            DispatchQueue.main.async {
                ...
                if let dateStr = dataDic["beginTime"] as? String{
                    let date = Date(withDateStr: dateStr, withFormatStr: "H:mm")
                ...
                weatherSetting.cityCount = dataDic["cityCount"] as? Int ?? 0
                weatherSetting.isEmailOpen = dataDic["byEmail"] as? Bool ?? false
                weatherSetting.isWeixinOpen = dataDic["byWeixin"] as? Bool ?? false
                weatherSetting.isBindingEmail = dataDic["isBindingEmail"] as? Bool ?? false
                weatherSetting.isBindingWeixin = dataDic["isBindingWeixin"] as? Bool ?? false
                weatherSetting.wxNickName = dataDic["wxNickName"] as? String ?? ""
                weatherSetting.email = dataDic["email"] as? String ?? ""
    ...
    public func setWeatherSetting(withWeatherSetting weatherSetting: WeatherSetting , closure: @escaping (_ codeResult: CodeResult, _ message: String)->()){
        let dic = [
            ...
            "time": weatherSetting.date.formatString(with: "H:mm"),
            "status": weatherSetting.isPause ? "1" : "0",
            "byEmail":"\(weatherSetting.isEmailOpen)",
            "byWeixin":"\(weatherSetting.isWeixinOpen)"
        ]
    ...
    //MARK:- 根据定位添加城市
    public func setLocationCity(withCityLocation cityLocation: CLLocationCoordinate2D, withName name: NSString ,withCity city: NSString,withProvince province: NSString,closure: @escaping (_ codeResult: CodeResult, _ message: String)->()){
        let dic = [
    ...
    func getWeather(fromJSON jsonData: [String: Any]) -> WeatherModel{
        ...
        if let cityName = jsonData["city"] as? String{
            ...
            city.localFlag = (jsonData["localFlag"] as? Int ?? 0) == 1
            weather.city = city
        }
        ...
        weather.imgStr = jsonData["img"] as? String ?? "0"          //图片数字
